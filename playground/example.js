(function() {
	describe( "something", () => {
		...
		it( "something else", ( done ) => {
			...
			window.jsonldExpand = function ( object ) {
			...
			window.evalExpand = function( object ) {
				let libExpandPromise = jsonldExpand( object );
				let myExpandPromise = Carbon.JSONLD.Processor.expand( object );


				return Promise.all( [ libExpandPromise, myExpandPromise ] ).then( ( [ libExpand, myExpand ] ) => {
					expect( object ).not.toBe( libExpand );
					expect( object ).not.toBe( myExpand );

					expect( myExpand ).toEqual( libExpand );

					console.group( "Objects expanded -->" );
			...
			Promise.all( promises ).then( done ).catch( error => {
				console.error( error );
				done.fail( error );
			} );
			return;
