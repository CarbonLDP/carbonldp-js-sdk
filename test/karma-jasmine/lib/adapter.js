(function( window ) {
	...
	var getAllSpecNames = function( topSuite ) {
		...
		function processSuite( suite, pointer ) {
			...
			for( var i = 0; i < suite.children.length; i ++ ) {
				...
				child.description = JE.prettyPrint( child.description );
	...
	var KarmaReporter = function( tc, jasmineEnv ) {
		...
		this.suiteStarted = function( result ) {
			result.description = JE.prettyPrint( result.description );
		...
		this.specDone = function( specResult ) {
			...
			var result = {
				description: JE.prettyPrint( specResult.description ),
				id         : specResult.id,
				log        : [],
				skipped    : skipped,
