...
class Documents implements Pointer.Library, Pointer.Validator, ObjectSchema.Resolver {
	...
	createChild( parentURI:string, slugOrChildDocumentOrFile:any, childDocumentOrFileOrRequestOptions:any = {}, requestOptions:HTTP.Request.Options = {} ):Promise<[ Pointer.Class, HTTP.Response.Class ]> {
		let slug:string = Utils.isString( slugOrChildDocumentOrFile ) ? slugOrChildDocumentOrFile : null;
		let childDocumentOrBlob:any = ( ! slug ) ? slugOrChildDocumentOrFile : childDocumentOrFileOrRequestOptions;
		...
		if ( Document.Factory.hasClassProperties( childDocumentOrBlob ) ) {
			let childDocument:Document.Class = childDocumentOrBlob;
