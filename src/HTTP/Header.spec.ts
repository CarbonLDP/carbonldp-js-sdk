...
describe( "Header", () => {
	...
	describe( "Header.parseHeaders", () => {
		...
		it( "should join multiple same header", () => {
			const headersString:string = `
				Accept: text/html
			...
			expect( headersMap.get( "accept" ) ).toEqual( new Header( [
				"text/html",
		...
		it( "should throw error if invalid string", () => {
			expect( () => Header.parseHeaders( `invalid string` ) )
				.toThrowError( IllegalArgumentError, "The header couldn't be parsed." );
		} );
