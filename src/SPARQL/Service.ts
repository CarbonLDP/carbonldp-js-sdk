...
export class Class {
	...
	static executeSELECTQuery<T>( url:string, selectQuery:string, pointerLibrary:Pointer.Library, options:HTTP.Request.Options = {} ):Promise<[ SELECTResults.Class<T>, HTTP.Response.Class]> {
		return Class.executeRawSELECTQuery( url, selectQuery, options ).then( ( [ rawResults, response ]:[ RawResults.Class, HTTP.Response.Class ] ):[ SELECTResults.Class<T>, HTTP.Response.Class] => {
