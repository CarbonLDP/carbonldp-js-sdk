...
describe( module( "carbonldp/QueryDocuments/QueryValue2" ), ():void => {

	describe( clazz( "CarbonLDP.QueryDocuments.QueryValue2", "Class that represents a property in the query" ), ():void => {

		it( "should exists", ():void => {
			expect( QueryValue2 ).toBeDefined();
			expect( QueryValue2 ).toEqual( jasmine.any( Function ) );
		} );
		...
		describe( constructor(), ():void => {
			...
			it( "should exists", ():void => {
				const queryValue:QueryValue2 = new QueryValue2( queryContainer, "value" );
				expect( queryValue ).toEqual( jasmine.any( QueryValue2 ) );
			} );
			...
			it( "should create a literal token when number", ():void => {
				const queryValue:QueryValue2 = new QueryValue2( queryContainer, 1 );
				expect( queryValue[ "_literal" ] ).toEqual( jasmine.any( LiteralToken ) );
			} );

			it( "should create a literal token when boolean", ():void => {
				const queryValue:QueryValue2 = new QueryValue2( queryContainer, true );
				expect( queryValue[ "_literal" ] ).toEqual( jasmine.any( LiteralToken ) );
			} );

			it( "should create a literal token using withType when Date", ():void => {
				const spy:jasmine.Spy = spyOn( QueryValue2.prototype, "withType" ).and.callThrough();

				new QueryValue2( queryContainer, new Date() );
		...
		describe( method( INSTANCE, "withType" ), ():void => {
			...
			it( "should exists", ():void => {
				expect( QueryValue2.prototype.withType ).toBeDefined();
				expect( QueryValue2.prototype.withType ).toEqual( jasmine.any( Function ) );
			} );
			...
			it( "should return the object", ():void => {
				const helper:( value:any, type:string ) => void = ( value, type ) => {
					const queryValue:QueryValue2 = new QueryValue2( queryContainer, value );
					const returnedValue:QueryValue2 = queryValue.withType( type );
		...
		describe( method( INSTANCE, "withLanguage" ), ():void => {
			...
			it( "should exists", ():void => {
				expect( QueryValue2.prototype.withLanguage ).toBeDefined();
				expect( QueryValue2.prototype.withLanguage ).toEqual( jasmine.any( Function ) );
			} );
		...
		describe( method( INSTANCE, "getToken" ), ():void => {
			...
			it( "should exists", ():void => {
				expect( QueryValue2.prototype.getToken ).toBeDefined();
				expect( QueryValue2.prototype.getToken ).toEqual( jasmine.any( Function ) );
			} );
		...
		describe( method( INSTANCE, "toString" ), ():void => {
			...
			it( "should override the default toString", ():void => {
				expect( QueryValue2.prototype.toString ).not.toBe( Object.prototype.toString );
			} );
